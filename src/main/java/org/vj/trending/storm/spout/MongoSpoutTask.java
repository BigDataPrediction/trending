...
class MongoSpoutTask implements Callable<Boolean>, Runnable, Serializable
{
    ...
    static Logger LOG = Logger.getLogger(org.vj.trending.storm.spout.MongoSpoutTask.class);
    ...
    private void initializeMongo(String url, String dbName)
    {
        ...
        {
            // Log the error
            LOG.info("Unknown host for Mongo DB" + e.getMessage(), e);
            // Die fast
    ...
    public Boolean call() throws Exception
    {
        ...
        this.cursor = this.collection.find(query).sort(new BasicDBObject("$natural", 1)).addOption(Bytes.QUERYOPTION_TAILABLE)
        ...
        while (running.get())
        {
            ...
            {
                LOG.info("Mongo Error: " + e.getMessage(), e);
                if (running.get())
                    throw new RuntimeException(e);
            }
        }
    ...
    public void run()
    {
        ...
        {
            LOG.info("Mongo Error: " + e.getMessage());
        }
    }
}